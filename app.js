...
game.party.push(pokemon[10], pokemon[25], pokemon[90]);
...
if (starterIndex !== -1) {
  const evolved = pokemon.find(p => p.number === evolvedMap[game.party[starterIndex].number]);
...
pokemon.forEach(p => {
  if (p.starter) console.log(p.name);
});
